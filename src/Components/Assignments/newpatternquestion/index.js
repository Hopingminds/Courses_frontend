...
export default function NewQuestion() {
  ...
  async function Fetchdata() {
    try {
      let url = `${BASE_URL}/getassesmentquestion?moduleAssessmentid=${params.get(
        "moduleAssessmentid"
      ...
      if (response.success) {
        ...
        Object.keys(response?.ModuleAssessment?.ProctoringFor).forEach(
          (key) => {
            newProctoringActive[key] =
              response?.ModuleAssessment?.ProctoringFor[key].inUse;
          }
        );
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (response.success) {
        ...
        setSelected("");
        navigate(
          `/question?moduleAssessmentid=${params.get(
            "moduleAssessmentid"
          )}&index=${index + 1}&t=${params.get("t")}`
        );
      }
    } catch (error) {
      console.log(error);
