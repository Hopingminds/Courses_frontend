...
export default function AiJourney() {
    ...
    return (
        <div id="journey" className="px-[5%] py-[4%] pl-[24%] bg-[#EBEBEB] flex flex-col gap-2 xsm:px-[2%] xsm:py-[2%] xsm:gap-1">
            <div className="bg-[#F2F2F2] w-[97%] shadow-xl">
                <p className="font-mons font-extrabold text-[30px] text-[#169f63]  px-8 pt-12 xsm:text-[14px] xsm:pt-2 xsm:px-2">How Your Journey Will Look Like</p>
                <p className="font-mons font-medium text-[17px] text-[#100101]  px-8 pt-4 xsm:text-[8px] xsm:pt-0 xsm:px-2 ">We Have Designed A Unique Program Where You Can Learn Data Science & Experience Being A Data Engineer Or Software Developer</p>
                <div className="grid grid-cols-8 text-left mt-10 ainavbtn xsm:mt-3">
                    {Object.keys(components).map(component => (
                        <button
                            key={component}
                            className={`text-center text-[12px] font-mons font-semibold px-[25px] py-[20px] xsm:text-[5px] xsm:px-1 xsm:py-2 ${activeOne === component ? 'active' : '' }`}
                ...
                {activeOne && (
                    <div className="bg-[#169F63] flex flex-col p-[20px] items-center xsm:p-2">
                        <p className="font-mons text-white text-[14px] xsm:text-[7px]">{components[activeOne].text}</p>
                        <div className="bg-white w-full flex flex-col items-center mt-4 xsm:mt-1">
                            <p className="text-[18px] text-[#777d78] font-mons font-bold xsm:text-[10px]">Opportunities it opens</p>
