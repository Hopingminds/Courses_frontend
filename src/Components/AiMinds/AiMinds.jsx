...
export default function AiMinds() {
    ...
    useEffect(() => {
        function handleScroll() {
            ...
            if (overview && tools && journey && benefits && hm && opportunities && project ) { 
                ...
                // var expertsTop = experts.getBoundingClientRect().top;
                // var expertsBottom = experts.getBoundingClientRect().bottom;
    
                if (overviewTop <= windowHeight / 2 && toolsTop > windowHeight / 2) {
                    setSidebarVisible(true);
                    setActiveSection('overview');
                } else if (toolsTop <= windowHeight / 2 && journeyTop > windowHeight / 2) {
                    setSidebarVisible(true);
                    setActiveSection('tools');
                } else if (journeyTop <= windowHeight / 2 && benefitsTop > windowHeight / 2) {
                    setSidebarVisible(true);
                    setActiveSection('journey');
                } else if (benefitsTop <= windowHeight / 2 && hmTop > windowHeight / 2) {
                    setSidebarVisible(true);
                    setActiveSection('benefits');
                } else if (hmTop <= windowHeight / 2 && opportunitiesTop > windowHeight / 2) {
                    setSidebarVisible(true);
                    setActiveSection('hm');
                } else if (opportunitiesTop <= windowHeight / 2 && projectTop > windowHeight / 2) {
                    setSidebarVisible(true);
                    setActiveSection('opportunities');
                } else if (projectTop <= windowHeight / 2 && projectBottom > windowHeight ) {
                    setSidebarVisible(true);
                    setActiveSection('project');
                } 
                // else if (expertsTop <= windowHeight / 2 && expertsBottom > windowHeight ) {
                //     setSidebarVisible(true);
        ...
        <div className="">
            ...
            <div className="relative">
                <div id="sidebar" className={`bg-[#169F63] text-white flex flex-col py-4 shadow-xl full-shadow xsm:hidden ${sidebarVisible ?'fixed top-[30%] left-[6%] 2xl:left-[22%]' :'hidden' }`}>
                    ...
                    <a href="#benefits" className={`text-[15px] font-mons px-4 py-1 ${activeSection === 'benefits' ? 'border-l-4 border-white' : ''}`}>Guaranteed Growth</a>
                    <a href="#hm" className={`text-[15px] font-mons px-4 py-1 ${activeSection === 'hm' ? 'border-l-4 border-white' : ''}`}>Benefits With HM</a>
                    <a href="#opportunities" className={`text-[15px] font-mons px-4 py-1 ${activeSection === 'opportunities' ? 'border-l-4 border-white' : ''}`}>Job Opportunities</a>
                    <a href="#project" className={`text-[15px] font-mons px-4 py-1 ${activeSection === 'project' ? 'border-l-4 border-white' : ''}`}>Project Powerhouse</a>
                    {/* <a href="#experts" className={`text-[15px] font-mons px-4 py-1 ${activeSection === 'experts' ? 'border-l-4 border-white' : ''}`}>Experts’ Viewpoint</a> */}
                </div>
                ...
                <AiOpportunities onSectionEnter={() => onSectionEnter('opportunities')} />
                <AiProject onSectionEnter={() => onSectionEnter('project')} />
                {/* <AiExperts onSectionEnter={() => onSectionEnter('experts')} /> */}
            </div>
        </div>
    );
}