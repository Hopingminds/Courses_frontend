...
const InternshipBatchModal = ({ isOpen, onRequestClose,Data ,fetchUserData}) => {
...
async function handleChoose(batchid,courseid){
    try {
        const data=await fetch(BASE_URL+'/setInternshipBatchForStudent',{
            method:"POST",
            headers:{
                'Content-type':'application/json',
                Authorization:`Bearer ${localStorage.getItem('COURSES_USER_TOKEN')}`
            },
            body:JSON.stringify({internshipId:courseid,batchId:batchid})
    ...
    >
      <div className="bg-white p-6 rounded-lg shadow-lg  w-full cursor-pointer">
...

        {
            ...
            <button className='bg-black text-white p-2 mx-auto rounded' onClick={()=>handleChoose(item?._id,item?.internship)}>Choose</button>
        </div>
                </>)
            })
        }
...
export default InternshipBatchModal;
