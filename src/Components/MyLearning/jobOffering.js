...
const JobOffering = ({ courses }) => {
    ...
    return ( <
        ...
        <
        div className = { `flex flex-col justify-between sm:w-[100%] xsm:w-[100%] md:w-[65%] md:gap-3 ${
...

        {
            jobOpeningData ? .map((item, ind) => {
                ...
                return ( <
                    ...
                    <
                    img className = "w-full h-full rounded-xl xsm:rounded-lg object-cover "
                    src = { item ? .logoUrl }
                    /> <
                    /div> <
                    ...
                    <
                    p className = "font-pop font-semibold text-[18px] xsm:text-[12px] md:text-[16px] sm:text-[13px]" > { item ? .position } <
                    /p> <
                    p className = "font-pop text-[#555555] text-[13px] xsm:hidden md:text-[10px] sm:text-[10px]" > { item ? .company } <
                    /p> <
                    ...
                    <
                    img className = "w-[16px] h-[16px] xsm:w-3 xsm:h-3 md:w-4 md:h-4 object-cover"
                    src = "../Icons/RCDesign.svg" /
                    >
                    <
                    p className = "font-pop text-[11px] font-medium text-[#555555] xsm:text-[8px] md:text-[10px] sm:text-[10px]" > { item ? .work_mode } <
                    /p> <
                    /div> {
                        item.salaryType == "Salary Range" ? < p className = " text-sm" > ₹{ item ? .annual_salary_range.from } - { item ? .annual_salary_range.to }
                        LPA < /p>:
                        item.salaryType == "uptoPackage" ? < p className = " text-sm" > Upto₹ { item ? .uptoPackage }
                        LPA < /p>: <
                            p className = "text-sm sm:text-[10px]" > ₹{ item ? .annualSalary }
                        LPA < /p>
                    } <
                    /div> <
                    /div> <
                    ...
                    p className = "text-[14px] sm:text-[12px] font-semibold" > Skills required < /p> {
                        item ? .key_skills ? .map((it) => {
                            return <p className = "font-pop capitalize text-[11px] font-medium text-[#555555] xsm:text-[8px] md:text-[10px] sm:text-[10px]" > { it }, <
                                /p>
                        })
                    }
                    ...
                    <
                    p className = "font-pop text-[14px] font-medium text-[#555555] xsm:text-[8px] md:text-[10px] sm:text-[10px]" > { formatDate(item ? .lastDate) } <
                    /p> <
                    /div>
                    ...
                    /div> <
                    div className = "flex flex-col w-[25%] justify-start gap-3 justify-self-start sm:justify-self-end items-start md:mt-2" > {!item ? .isApplied ? ( <
                            button onClick = {
                                (e) => handleApply(e, item ? ._id) }
                    ...
                    <
                    button onClick = {
                        () => navigate("/jobpreview?jobid=" + item ? ._id) }
