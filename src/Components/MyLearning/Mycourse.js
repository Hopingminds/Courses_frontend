    ...
    return (
        <div className="my-10 mx-[5%] flex flex-wrap justify-between xsm:my-2">

            {
                courses?.map((val, ind) => {
                    return (
                        <Link to={`/course/${val.slug}`} className="w-[29%] px-4 py-6 mt-2 rounded-xl shadow-xl shadow-[#D9D9D9] xsm:mt-0 xsm:py-1 xsm:px-1 xsm:rounded-sm">
                            <div>
                                <img className="w-full h-full xsm:rounded-sm xsm:h-[55px]" src={val?.featured_image} />
                            </div>
                            <div className="space-y-3 mt-2 xsm:space-y-1 xsm:mt-1">
                                <p className="font-pop font-semibold text-[18px] xsm:text-[6px]">{val?.title}</p>
