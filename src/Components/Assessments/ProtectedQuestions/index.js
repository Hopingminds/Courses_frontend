...
export default function ProtectedAssessmentQuestion() {
  ...
  async function Fetchdata() {
    try {
      let url = `${BASE_URL}/getassessment?assessmentId=${params.get(
        "assessmentId"
      ...
      if (response.success) {
        ...
        if (!response?.isProtected) {
          window.location.replace(
            `/assessmentquestions?assessmentId=${params.get(
              "assessmentId"
            )}&index=${1}`
          );
        }
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (response.success) {
        ...
        setSelected("");
        navigate(
          `/prassessmentquestions?assessmentId=${params.get(
            "assessmentId"
          )}&index=${index + 1}`
        );
      }
                  ...
                  {data?.options &&
                    data?.options?.map((item) => (
                      <label
                        key={item?._id}
                        onClick={() =>
                          !data?.isSubmitted ? setSelected(item?.option) : ""
                        }
                        htmlFor={item?.option}
                        className={`${
                          Selected === item.option ||
                          data?.submittedAnswer === item?.option
                            ? "border-[#1DBF73]"
                            : ""
